...
`Pretendard-Regular.ttf` is a 52KB static 400-weight subset generated from the
bundled `PretendardVariable.woff2` for use by Next OG's Satori renderer. If the
OG copy changes, regenerate this subset so every new glyph is included.

Reproduce the conversion with FontTools 4.60+:
...
font = TTFont('src/styles/fonts/PretendardVariable.woff2')
font = instantiateVariableFont(font, {'wght': 400}, inplace=False)
font.flavor = None
font.save('/tmp/Pretendard-Regular-full.ttf')
PY

python3 -m fontTools.subset /tmp/Pretendard-Regular-full.ttf \
  --text='raven.kr 생각을 다듬고 시스템으로 만듭니다 제품과 소프트웨어를 만들며 배운 것을 기록합니다.' \
  --unicodes='U+0000-007F' --layout-features='*' \
  --output-file=src/styles/fonts/Pretendard-Regular.ttf
```

The source font is Pretendard by Orion Cactus, distributed under the SIL Open
Font License 1.1. The license text is included below.
