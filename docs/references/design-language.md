...
글 상세도 레일은 왼쪽 사이드바 하나다. 위에 주제 내비게이션(사이트 안 위치), 그 아래 목차(글 안
위치)를 둔다. 오른쪽 레일은 두지 않는다. 900 이하에서 셸이 한 열로 접히고 사이드바가 사라지면
목차는 본문 위 접이식 하나로 옮긴다. 사이드바가 보이는 동안에는 본문 위 목차를 띄우지 않는다 —
어느 폭에서도 목차는 한 벌만 보인다. 사이드바의 sticky 정지선은 `--rail-top`이다.

640 이하에서 좁아지는 좌우 여백은 `--outer`가 낸다. 셸이 폭을 따로 깎으면 `padding-inline` 위에
거터가 한 겹 더 붙어 본문이 워드마크보다 안쪽에 선다.
...
- 한 글은 홈에 한 번만 나온다. 추천 → 최근 글 → 카테고리 섹션 순서로 자리를 주고, 앞에서
  나온 글은 뒤 섹션에서 뺀다. 빈 섹션은 그리지 않는다(`planHomeSections`).
- 추천 캐러셀의 텍스트는 이미지 윗변에 맞추고, 읽는 순서는 카테고리 → 제목 → 요약 → 날짜 →
  컨트롤이다. 컨트롤이 제목보다 먼저 읽히지 않게 한다.
- 캐러셀 이미지는 데스크톱 1.9:1, 640 이하 16:10이다. 모바일 첫 화면에 제목이 들어와야 한다.
- 한 카테고리만 보는 목록에서는 카드의 카테고리 eyebrow를 숨긴다. 여러 카테고리가 섞인
  목록(최근 글, 전체 글)에서만 보인다.
