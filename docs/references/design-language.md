...
섹션 배경을 바꿔 구획하지 않는다. 실제로 히어로를 `--color-text`로 뒤집어 봤더니
**캔버스 대비가 16.73:1**(가능한 최대치)이라 스크롤할 때 흑백이 슬램처럼 부딪혔다.

진폭을 낮춘 톤 패널도 답이 아니다 — 캔버스 대비 1.18:1짜리 옅은 패널에서도
그 위의 `--color-text-muted`가 4.0:1로 **AA 미달**이 된다. 배경을 하나 두는 순간
그 배경 전용 색 세트가 항상 따라붙는다.

구획은 hairline(`--color-border`)과 여백이 맡고, 무게는 조판(`--fs-display`)이 만든다.

> 되살릴 일이 생기면 알아둘 것: 토큰을 다시 묶는 것만으로는 부족하고 `color`를 함께 명시해야
> 한다. `body`에서 이미 계산된 `color`는 상속될 뿐 재평가되지 않아, 색 선언이 없는 자식(`h1` 등)이
> 어두운 글자 그대로 남아 사라진다. 실제로 이 실수를 한 번 냈다.

## 타이포 토큰

- 스케일: `--fs-56 … --fs-13`(px→rem 환산). `--fs-72`는 `--fs-display`로 대체돼 삭제했다.
- **디스플레이 스케일**: `--fs-display`(`clamp(2.75rem, 8vw, 8rem)`) ·
  `--fs-display-sm`(`clamp(2rem, 5.5vw, 5rem)`). 뷰포트를 따라 늘어나 화면 폭을 실제로 채운다.
  랜딩 헤드라인과 푸터 워드마크 전용 — 고정 스케일로는 1400px 컨테이너에서 글자가 작아 보여
  화면이 와이어프레임처럼 읽혔다.
...
- 풀블리드 섹션(반전 블록)은 배경을 화면 끝까지 두고 **`Container`를 자기 안에** 둔다 —
  배경은 화면 폭, 글은 다른 페이지와 같은 격자.
