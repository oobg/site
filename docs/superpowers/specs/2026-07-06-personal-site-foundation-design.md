...
src/
  ...
  features/
    content/
      services/
        content.api.ts      # 서버 read 함수: getPosts/getPost/getProjects/getProject (canonical)
        content.query.ts    # TanStack Query queryOptions (상호작용 페이지의 prefetch/useQuery용)
      types/
        content.types.ts    # 계약 타입 + ProjectFrontmatter 등 헬퍼
      fixtures/
        content.mock.ts     # 계약 예시 JSON 픽스처 (mock 소스 + 테스트 공용)
      components/            # 콘텐츠 전용 재사용 컴포넌트(PostRow 등, 페이지 마일스톤에서 확장)
  components/
  ...
  lib/
    api/http.ts             # 서버 전용 envelope fetch 래퍼(캐시 태그/재검증, 에러/404 처리)
...
### 타입 (`features/content/types/content.types.ts`) — `content-v1.md` 미러
...
export interface ProjectFrontmatter {             // frontmatter에서 읽는 헬퍼
...
### 콘텐츠 read 함수 (`features/content/services/content.api.ts`) — canonical

`getPosts(params)`, `getPost(slug)`, `getProjects(params)`, `getProject(slug)`.
`@configs/env`의 `CONTENT_SOURCE`(`mock`|`api`, 기본 `mock`)로 분기:
- `mock`: `fixtures/content.mock.ts` 반환(백엔드 콘텐츠 API 미구현 대응).
- `api`: `lib/api/http.ts` 경유. 백엔드 준비 후 env만 `api`로 스왑.
- 정적 RSC 페이지는 이 함수를 서버에서 직접 호출.

### TanStack Query 옵션 (`features/content/services/content.query.ts`)

`postsQueryOptions(params)`, `postQueryOptions(slug)` 등 `queryOptions`(queryKey + queryFn=위 read
함수). 상호작용 페이지의 서버 prefetch + 클라이언트 `useQuery`용. `staleTime` 높게(콘텐츠는 웹훅으로
무효화되므로 클라이언트 재요청 최소). 이번 마일스톤은 정의만, 실제 사용은 상호작용 페이지 등장 시.
- 주의: `content.api`는 `server-only`(lib/api/http)라 클라이언트 번들에 들어갈 수 없다. 클라이언트
...
- **최근 생각(Latest Thinking) 1건**: 서버에서 `getPosts({ limit: 1, sort: '-published_at' })` 직접
  호출 → 최신 글 제목 + `ArrowLink`. 데이터 계층 엔드투엔드 검증 지점.
- **미니멀 푸터**: 텍스트 링크 소수.
...
1. **계약 적합성 겸 파싱 테스트**: `content.api`가 `content.mock`/예시 JSON을 올바르게 언랩·타이핑.
   (fetch 모킹으로 `api` 경로도 1케이스.)
