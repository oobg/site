...
dev 자동 배포 workflow는 위 내부 검증과 rollback 결과를 반영해 `.github/workflows/deploy-dev.yml`에 작성했다. trigger는 `dev` push와 수동 실행으로 제한하고, 입력 SHA가 `origin/dev`에 포함됐는지 확인한다. 기존 main push workflow는 변경하지 않았다. workflow 파일은 아직 commit·push하지 않아 원격에서 활성화되지 않았다.
...
현재 checkout에서는 앱 129 tests, typecheck, lint(기존 warning 2건), dev metadata Next build가 통과했다. dev 전용 Dockerfile의 실제 local build와 read-only root container `/health` smoke도 통과했다. 배포 shell harness의 success, build failure, up failure, health failure, rollback, first-deploy 시나리스를 검증했고 홈서버에서도 실제 app health 실패 후 이전 이미지를 복구했다. 최신 배포 이미지는 `snapshot-d57de14f186eb41f`이며 source digest는 `d57de14f186eb41fc85d9324c07f2b749266a43d233895260570a94fd937c1fc`다.

Cloudflare tunnel version 12와 proxied DNS로 세 dev origin을 공개했다. 공개 origin에서는 health와 `/blog` SSR, GIS 로그인, Google 계정 선택, GoTrue 교환, owner 관리자 진입을 확인했다. 고유 draft는 공개 목록에서 보이지 않았고 발행 뒤 공개 상세에 title과 body가 표시됐다. local PNG 업로드는 `/assets/posts/...` Markdown 경로를 만들었고 `cdn-dev.raven.kr`의 WebP 폭 변환도 확인했다. acceptance용 database row와 source asset을 삭제한 뒤 공개 상세와 cache-bust CDN 요청이 404임을 확인했다. Day0 공개 Supabase origin은 기존 no-key 응답을 유지한다. `.github/workflows/deploy-dev.yml`은 위 검증을 재현하도록 로컬에 작성했지만 아직 commit·push하지 않아 원격에서 활성화되지 않았다.
